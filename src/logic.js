...
import {
  ...
  getHighestOffer,
  getBankerOptions,
  getPreviousBankerOffers
} from './reducer';
import _ from 'lodash';

const genCasesToOpenInfoText = num => `Chose ${num} more case(s) to open`;
const roundsSequence = [6, 5, 4, 3, 2, 1, 1, 1, 1, 1];
const getTargetOpenCases = currRound =>
  roundsSequence.slice(0, currRound).reduce((n, total) => n + total);
...
const caseClickedLogic = createLogic({
  ...
  process({ getState, action }, dispatch, done) {
    ...
    if (currentRound === 0) {
      ...
      dispatch({
        type: 'CHANGE_INFO_TEXT',
        payload: genCasesToOpenInfoText(roundsSequence[currentRound])
      });
      ...
      //Need To call Banker
        ...
        dispatch({
          type: 'CHANGE_INFO_TEXT',
          payload: genCasesToOpenInfoText(desiredCasesToOpen)
        });
      }
    }
    done();
...
const bankerCallLogic = createLogic({
  ...
  process({ getState, action }, dispatch, done) {
    ...
    const remainingMean =
      unopenedCases.reduce((a, b) => a + b.mask, 0) / unopenedCases.length;
    const multiplier = bankerOfferDecay * currentRound;
    const bankOffer = Math.ceil(
      multiplier < 1 ? remainingMean * multiplier : remainingMean
    );
    let realOffer = '';
    ...
    switch (true) {
      case bankOffer > 0 && bankOffer <= 35000:
        realOffer = getRealOffer(bankerOptions.small);
        break;
      case bankOffer > 35000 && bankOffer <= 65000:
        realOffer = getRealOffer(bankerOptions.mediumSmall);
        break;
      case bankOffer > 65000 && bankOffer <= 100000:
        realOffer = getRealOffer(bankerOptions.medium);
        break;
      case bankOffer > 100000 && bankOffer <= 150000:
        realOffer = getRealOffer(bankerOptions.mediumLarge);
        break;
      default:
        realOffer = getRealOffer(bankerOptions.large);
        break;
    }
    setTimeout(() => {
      dispatch({
        type: 'SET_BANKER_OFFER',
        payload: { bankerOffer: { maskOffer: bankOffer, realOffer: realOffer } }
      });
...
const bankerOfferDecisionLogic = createLogic({
  ...
  process({ getState, action }, dispatch, done) {
    ...
    if (action.payload.type === 'NO_DEAL') {
      if (currentRound === 9) {
        dispatch({
          type: 'CHANGE_INFO_TEXT',
          payload: 'Choose one of the remaining cases to eliminate!'
        });
      } else {
        dispatch({
          type: 'CHANGE_INFO_TEXT',
        ...
        dispatch({type: 'SHOW_REWARDS', payload: action.payload.offer})
    }
    dispatch({ type: 'INCREASE_ROUND' });
    done();
  }
...
const showRewardsLogic = createLogic({
  ...
  process({ getState, action }, dispatch, done) {
    ...
    if (typeof action.payload === 'object') {
      const rewardsArray = [];
      _.forOwn(action.payload, (val, key) => {
        if (typeof val === 'object') {
          rewardsArray.push(
            `${key.charAt(0).toUpperCase() + key.slice(1)}: Locations include ${val.location.join(
              ', '
            )}\nDescription: ${val.description}`
          );
        }
      });
      rewardString = rewardsArray.join('\n\n');
    } else {
      rewardString = action.payload;
    }
    dispatch({ type: 'SET_CHOSEN_REWARD', payload: rewardString });
